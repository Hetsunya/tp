...
if __name__ == '__main__':
    ...
    players.append(Player(name=username, is_ai=True, auto_ship=True, skill=1))
    players.append(Player(name='AI1', is_ai=True, auto_ship=True, skill=1))
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            shot_result, sx, sy = game.current_player.make_shot(game.next_player)
            if shot_result == 'miss':
                ...
                shot_coordinates = '{}{}'.format(Game.letters[sx], sy + 1)
                game.current_player.message.append('Отличный выстрел в {}, продолжайте!'.format(shot_coordinates))
                game.next_player.message.append('Ваш корабль в {} попал под обстрел!'.format(shot_coordinates))
                continue
